...
pub const GitNote = struct {
    ...
    pub fn create(
        ...
        log.debug("Created note for commit {s} in namespace {s}", .{ commit_id.str(), notes_ref });
    ...
    pub fn read(repo: GitRepository, commit_id: GitOID, notes_ref: []const u8) !?GitNote {
        ...
        if (res < 0) {
            ...
            log.err("Failed to read note from namespace {s} with error code: {d}", .{ notes_ref, res });
    ...
    pub fn update(
        ...
        log.debug("Updated note for commit {s} in namespace {s}", .{ commit_id.str(), notes_ref });
        return GitOID{ .value = note_oid };
    }
    ...
    pub fn remove(repo: GitRepository, commit_id: GitOID, signature: GitSignature, notes_ref: []const u8) !void {
        ...
        if (res < 0) {
            ...
            log.err("Failed to remove note from namespace {s} with error code: {d}", .{ notes_ref, res });
            return GitError.NOTE_DELETE_FAILED;
        }

        log.debug("Removed note for commit {s} from namespace {s}", .{ commit_id.str(), notes_ref });
    }
