...
pub const Slice = struct {
        ...
        for (slices) |*s| {
            ...
            if (s.getParentFromNotes(alloc)) |maybe_parent_branch_name| {
                if (maybe_parent_branch_name) |parent_branch_name| {
                    ...
                    if (!parent_found) {
                        log.debug(
                            "Parent {s} from git notes not found in current slice list for {s}",
                            .{ parent_branch_name, s.name() },
                        );
                    }
                } else {
    ...
    pub fn createParentNote(self: *Slice, parent_branch: []const u8, alloc: Allocator) !void {
        ...
        // Format note content
        const note_content = try std.fmt.allocPrint(
            alloc,
            "{s}{s}",
            .{ constants.SLICE_PARENT_NOTE_PREFIX, parent_branch },
        );
    ...
    pub fn updateParentNote(self: *Slice, new_parent_branch: []const u8, alloc: Allocator) !void {
        ...
        const note_content = try std.fmt.allocPrint(
            alloc,
            "{s}{s}",
            .{ constants.SLICE_PARENT_NOTE_PREFIX, new_parent_branch },
        );
        defer alloc.free(note_content);
...
fn isValidBranchReference(branch_name: []const u8) bool {
    ...
    if (std.mem.startsWith(u8, branch_name, ".") or std.mem.endsWith(u8, branch_name, ".")) {
        return false; // No leading/trailing dots
    }
    if (std.mem.endsWith(u8, branch_name, "/") or std.mem.startsWith(u8, branch_name, "/")) {
